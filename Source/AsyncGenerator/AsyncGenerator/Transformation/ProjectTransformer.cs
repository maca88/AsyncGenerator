	...
	public class ProjectTransformer
	{
		...
		private TypeTransformationResult TransformType(ITypeAnalyzationResult rootTypeResult)
		{
			...
			foreach (var typeResult in rootTypeResult.GetSelfAndDescendantsTypes(o => o.Conversion != TypeConversion.Ignore))
			{
				...
				{
					foreach (var reference in typeResult.TypeReferences)
					{
		...
		private MethodTransformationResult TransformMethod(TypeTransformationMetadata typeMetadata, IMethodAnalyzationResult methodResult)
		{
			...
			// First we need to annotate nodes that will be modified in order to find them later on. We cannot rely on spans after the first modification as they will change
			var typeReferencesAnnotations = new List<string>();
			foreach (var reference in methodResult.TypeReferences)
			{
			...
			foreach (var referenceResult in methodResult.MethodReferences.Where(o => o.GetConversion() == FunctionReferenceConversion.ToAsync))
			{
